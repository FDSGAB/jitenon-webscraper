...
class LastNameInformationFetcher():
    ...
    def fetch_information_from_table(self, content : BeautifulSoup):
      table_of_contents_rows = content.find("div", {'class':'myoji_box'}).find('ul').find_all('li')
      for row in table_of_contents_rows:
          last_name = row.find("h3").find('a').get_text()
          yomi = row.find("p").find('a').get_text()
          self.information_list.append(['名字', last_name + '◦' + yomi])
